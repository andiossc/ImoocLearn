...
//    return new Data(a.date).getTime() > new Date(b.date).getTime();
//})
//
////格式化数据
...
//        }
//    }
//    html_scrubber.unshift(
//        tpl_scrubber_year.replace(/\{year\}/g, year).replace(/\{list\}/g, scrubber_month.join(''))
//    )
//    html_content.unshift(
//        tpl_content_year.replace(/\{year\}/g, year).replace(/\{list\}/g, content_month.join(''))
//    )
//}
//
////写入内容
//g('scrubber').innerHTML = '<a href="javascript:;" onclick="scrollTo(0)">现在</a>' + html_scrubber.join('') +
//    '<a href="javascript:;" onclick="scrollTopTo(getBodyH())">出生</a>';
//g('content').innerHTML = html_content.join('') + tpl_content_year.replace(/\{year\}/g, '出生')
//    .replace(/\{list\}/g, '') + '<div class="clear c_month" id="c_month_0_0"></div>';
//
////动画卷动
...
//        if (top > tops[i - 1] && top < top[i]) {
//            var year = year[i - 1].innerHTML;
